...
/**
 * Reads the compact grid written by `./dev.sh soilph` (uint8 south→north,
 * scale 0.1, 255 = unknown).
 */
final class EcoDataCubeSoilPhRaster implements SoilPhSource
{
    ...
    public function sample(Grid $grid): \SplFixedArray
    {
        ...
        if (!$this->isAvailable()) {
            $this->logger->info('pH EcoDataCube absent, repli Charm-50', [
        ...
        if (strlen($bytes) < $expected) {
            $this->logger->warning('Grille pH tronquée', ['bytes' => strlen($bytes), 'expected' => $expected]);

            return $out;
        }

        $this->logger->info('pH du sol depuis EcoDataCube', [
            'cells' => $expected,
            'path' => $this->binPath(),
        ]);
    ...
    {
        // Valid EcoDataCube bytes are roughly 30–95 (pH 3–9.5); 255 is nodata.
        return ($value === self::UNKNOWN || $value < 20 || $value > 100) ? -1 : $value;
    }
    ...
    private function sidecarPath(): string
    {
        ...
        return rtrim($this->datasetPath, '/').'/soil-ph.json';
    }
